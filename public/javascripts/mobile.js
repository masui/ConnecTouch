    ...
    function(data, status) { //第三引数に受信時の処理を記述
      if (status == 'success' && data != undefined) { //通信が成功し中身も存在する場合処理を開始
        ...
        for (index in data) {
          ...
          // }
          $('.timeline').append('<div class="column"><img class="icon" src="" alt="icon"/><div class="link" data-param="'+ data[index].link[0]+'"><span class="place">'+_place +'</span><span class="time">'+_time +'</span></div></div>');
